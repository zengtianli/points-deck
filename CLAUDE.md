...
操作顺序（⚠ 没有独立的「解锁」按钮，必须先凑出一笔账）：

1. 主界面**右上角 🔒** → 家长记账面板
2. 选科目 → 选事由 → 填数值，等服务端算出预览分
3. 点「**记这一笔**」→ 这时才弹密码框
4. 输 `Tianli@2026` → 点「解锁」
5. 之后连着记多少笔都不再问；顶部显示「已解锁 · 本次记了 N 笔」
6. 记完点右上角「**上锁**」

> ⚠ **已知交互缺陷**：`askPassword` 只在 `earn()` 里被置位（`ParentView.swift`），
> 而「记这一笔」按钮是 `.disabled(preview == nil)` —— 所以**想单纯解锁一下做不到**，
> 必须先选一条规则。`gateSection` 那栏只显示状态和「上锁」，**没有解锁按钮**。
> 修法：给 `gateSection` 加个解锁按钮，拿只读接口（如 `/api/config`）验密码后
> `session.unlock(pw)`，服务端不用动。
